...
import com.picc.bean.Function;
import com.picc.bean.sys.RacPrivilege;
import com.picc.bean.sys.RacRole;
import com.picc.dao.RacAcntRoleRelMapper;
import com.picc.dao.RacPrivilegeMapper;
import com.picc.dao.UserAccountMapper;
import com.picc.datasource.DataSourceContextHolder;
import com.picc.util.UrlPathMatcher;
...
public class MySecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
	...
	@Autowired
	private RacPrivilegeMapper privilegeMapper;
	...
	private void loadResourceDefine() {
		log.info("----4.MySecurityMetadataSource.loadResourceDefine");
		resourceMap = new HashMap<String, Collection<ConfigAttribute>>();
		DataSourceContextHolder.setDbType("dataSource2");  
		List<RacRole> roles = roleMapper.listAllRoles();
//		
//		for (RacRole res : roles) {
		RacRole res = null;
		for(int i = 0;i < roles.size();i++) {
			res = (RacRole)roles.get(i);
			log.info("------i:{}",i);
			
//			if(i>1) break;
			
			List<RacPrivilege> privilegeList = privilegeMapper.listPrivilegeByRoleId(res.getRacRoleId());
			
			
			//			List<Function> functions = this.functionDAO.findAllEffectiveFunctionsbyRoleId(role.getId());
			for (RacPrivilege privilege : privilegeList) {
				if("100000000".equals(privilege.getRacPrivilegeId())){
					log.info("1000000000");
				}
						
				
				Collection<ConfigAttribute> configAttributes = new ArrayList<ConfigAttribute>();
				ConfigAttribute configAttribute = new SecurityConfig(res.getCode());
				configAttributes.add(configAttribute);
				String url = privilege.getRpSysRemark();
	...
	public Collection<ConfigAttribute> getAttributes(Object object) throws IllegalArgumentException {
		log.info("----5.MySecurityMetadataSource.getAttributes");
		loadResourceDefine();
